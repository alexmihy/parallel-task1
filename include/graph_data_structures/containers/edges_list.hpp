...
{
	throw "ITERATE_ADJACENT_EDGES not implemented in EDGES LIST";
...
{
	throw "GET_EDGE_BY_ID not implemented in EDGES LIST";
	return Edge<_TEdgeWeight>(0, 0, 0);
}
...
{
	throw "GET_VERTEX_CONNECTIONS_COUNT not implemented in EDGES LIST";
	return 0;
}
...
EdgesList<_TVertexValue, _TEdgeWeight>* EdgesList<_TVertexValue, _TEdgeWeight>::clone() const
{
    ...
    EdgesList<_TVertexValue, _TEdgeWeight> *clone = new EdgesList<_TVertexValue, _TEdgeWeight>(vertices_count, edges_count, this->directed, true);
    
    universal_memcpy(clone->vertices.values, vertices.values, vertices_count + 1);
    
    universal_memcpy(clone->edges.src_ids, edges.src_ids, edges_count);
    universal_memcpy(clone->edges.dst_ids, edges.dst_ids, edges_count);
