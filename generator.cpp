...
bool save_to_file(int *_src_ids, int *_dst_ids, float *_weights, int _vertices_count, long long _edges_count, string _file_name, bool _weighted, bool _directed)
{
    ofstream graph_file(_file_name.c_str(), ios::binary);
    if (!graph_file.is_open())
        return false;
    
    graph_file.write(reinterpret_cast<const char*>(&_directed), sizeof(bool));
...
int main(int argc, char **argv)
{
    try
    {
        ...
        t1 = omp_get_wtime();
        save_to_file(src_ids, dst_ids, weights, vertices_count, edges_count, file_name, weighted, directed);
        t2 = omp_get_wtime();
